...
<?php
    ...
    $image3="<img class='image' src='img/sampo_logo.png' alt='Sampo logo'>";
    ...
    $fetch= mysqli_query($connection, "SELECT p_id, priceh, pricekm, c_id, rate, alkm FROM hb_provide WHERE s_id=7 OR s_id=10"); 
        
    while ($row = mysqli_fetch_assoc($fetch)){
        $kk = $row['p_id'];
        $cc = $row['c_id'];
        $fetch1 = mysqli_query($connection, "SELECT * FROM hb_company WHERE c_id = '$cc'");

        $row1 = mysqli_fetch_assoc($fetch1);
        $loc = $row1['longitude'];
        $lac = $row1['latitude'];
        echo "<div id='myBtn$kk' class='w3-center quarter3 kborder'>"; 
        echo ${"image" . $cc};
        echo "<div class='morepadding2'>";
        echo "<p class='boxk' style='font-weight:bold;'>" . $row1['c_name'] . "</p><p>" . $row['priceh'] ."€/h +". $row['pricekm'] . "€/km</p>";
        ///////// tähdet ///////////////////
...
?>
        ...
        <div id="myModal<?php echo $kk;?>" class="modal" data-modal=
            ...
            <div class="modal-content">
                <span class="close" data-modal="myModal<?php echo $kk;?>">&times;</span>
                <?php
                    $p_id = $row['p_id'];
                    $res_1= mysqli_query($connection, "SELECT s_id FROM hb_provide where p_id= '$p_id'");
                    $ff = mysqli_fetch_assoc($res_1);
                    $s_id = $ff['s_id'];
                    $res_2= mysqli_query($connection, "SELECT s_name FROM hb_service where s_id= '$s_id'");
                    $nimi = mysqli_fetch_assoc($res_2);

                ?>
                <div style="font-weight:bold;"><?php echo "<p>" . $nimi['s_name'] . "<p>";  ?></div>
